...
func (r *raft) tickElection() {
	...
	// 如果启用了边缘选举，根据节点评分调整选举超时
	if r.enableEdgeElection && r.nodeScore > 0 {
		// 根据节点评分调整选举超时
		// 评分越高，选举超时越短，越容易成为leader
		adjustedTimeout := int(float64(r.randomizedElectionTimeout) * (1.0 - r.nodeScore/100.0))
		if adjustedTimeout < r.heartbeatTimeout*2 {
			adjustedTimeout = r.heartbeatTimeout * 2
		}

		if r.electionElapsed >= adjustedTimeout {
			r.electionElapsed = 0
...
func stepCandidate(r *raft, m pb.Message) error {
	...
	case pb.MsgVote, pb.MsgPreVote:
		// 解析消息中的Context字段，获取发送方的节点评分
		var senderScore float64 = 0
		if len(m.Context) > 0 {
			// 尝试从Context中解析出发送方的节点评分
			if err := json.Unmarshal(m.Context, &senderScore); err == nil {
				// 成功解析出发送方的节点评分
				r.logger.Infof("收到来自节点 %x 的投票请求，其节点评分为 %f", m.From, senderScore)
			}
		}
		...
		// 如果启用了边缘选举，且发送方的评分明显高于本节点，可以考虑投票给它
		if r.enableEdgeElection && senderScore > r.nodeScore*1.2 {
			r.logger.Infof("节点 %x 的评分(%f)明显高于本节点(%f)，倾向于投票支持", m.From, senderScore, r.nodeScore)
			granted = true
		}
...
// 添加设置节点评分的方法
func (r *raft) SetNodeScore(score float64) {
	r.nodeScore = score
	r.logger.Infof("节点 %x 的评分已更新为 %f", r.id, score)
}

// 添加启用/禁用边缘选举的方法
func (r *raft) EnableEdgeElection(enable bool) {
	r.enableEdgeElection = enable
	r.logger.Infof("节点 %x 的边缘选举功能已%s", r.id, map[bool]string{true: "启用", false: "禁用"}[enable])
}
